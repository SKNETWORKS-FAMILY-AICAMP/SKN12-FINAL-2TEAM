...
export function AISignalCard({ signals }: { signals: AiSignal[] }) {
  return (
          ...
          return (
            <div key={s.label + i} className="flex flex-col gap-1 w-full">
              <div className="flex items-center justify-between w-full">
                ...
                <span className={`text-sm font-bold ${color}`}>{s.change ?? "N/A"}</span>
              </div>
              <div className="text-xs text-gray-400 leading-tight">
                {s.action} | {s.confidence} | {(s.reason ?? "").substring(0, 20)}...
              </div>
            </div>
          );
        })}
