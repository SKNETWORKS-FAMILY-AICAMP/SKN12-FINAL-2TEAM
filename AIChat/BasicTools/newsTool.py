...
class NewsTool(BaseFinanceTool):
    ...
    def get_data(self, keyword: str, max_results: int = 5) -> NewsOutput:
        params = {
            "q": keyword,
            "lang": "en",
            "token": self.api_key,
        ...
        if not articles:
            return NewsOutput(agent="error", summary=f"📭 '{keyword}'에 대한 뉴스를 찾을 수 없습니다.")
        ...
        summary = (
            f"📰 '{keyword}' 관련 최신 뉴스 {len(news_list)}건:\n" +
